...
exports.onPreBootstrap = async ({ actions, emitter, cache }, pluginOptions) => {
  ...
  if (emitter) {
    ...
    emitter.on(`CREATE_JOB_V2`, action => {
      if (action.plugin.name === `gatsby-plugin-sharp-test`) {
  ...
  Joi.object({
    ...
    defaults: Joi.object({
      ...
      placeholder: Joi.string().valid(
        //`tracedSVG`, // commented due to license banned issue
        `dominantColor`,
        `blurred`,
        `none`
      ),
     ...
     // tracedSVGOptions: Joi.object(), // commented due to license banned issue
